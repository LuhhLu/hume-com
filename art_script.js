...
function displayDateInRomanNumerals() {
    const date = new Date();
    const year = date.getFullYear();
    const month = date.getMonth() + 1; // JavaScript months are 0-11
    const day = date.getDate();

    const yearInRoman = convertToRomanNumerals(year);
    const monthInRoman = convertToRomanNumerals(month);
    const dayInRoman = convertToRomanNumerals(day);

    const dateElement = document.getElementById("year");
    dateElement.innerHTML = `${yearInRoman}/${monthInRoman}/${dayInRoman}`;
    dateElement.setAttribute('year-original', `${year}/${month}/${day}`);
}

document.addEventListener('DOMContentLoaded', function() {
    displayDateInRomanNumerals();

    const dateElement = document.getElementById("year");
    dateElement.addEventListener('mouseover', function() {
        dateElement.innerHTML = dateElement.getAttribute('year-original');
    });
...
// Call the function to set a random theme when the page loads
setRandomTheme();
